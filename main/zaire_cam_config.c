...
static zaire_err_t zaire_cam_XMP_init(void){
...
static zaire_err_t zaire_cam_start_cb(uvc_format_t format, int width, int height, int rate, void *cb_ctx){
    return ZAIRE_OK;
}

uvc_fb_t* zaire_cam_fb_get_cb(void *cb_ctx){
    (void)cb_ctx;
...
static void zaire_cam_fb_return_cb(uvc_fb_t *fb, void *cb_ctx){
    (void)cb_ctx;
    assert(fb == &s_fb.uvc_fb);
    esp_camera_fb_return(s_fb.cam_fb_p);
}

static void zaire_cam_stop_cb(void *cb_ctx){
    (void)cb_ctx;
...
zaire_err_t zaire_cam_init(void){
    ...
    if(zaire_cam_XMP_init() != ZAIRE_OK){
        ESP_LOGE("CAMERA", "Could not init camera");
        return ZAIRE_ERR;
    }    
    ...
    uvc_device_config_t config = {
        ...
        .start_cb = zaire_cam_start_cb,
        .fb_get_cb = zaire_cam_fb_get_cb,
        .fb_return_cb = zaire_cam_fb_return_cb,
        .stop_cb = zaire_cam_stop_cb,
    };

    zaire_err_t ret = uvc_device_config(0, &config);
    if(ret != ZAIRE_OK){
