...
{
    char settings_data[1024] = {0};

    // != first scope end point []
    // &= second scope end point [[]]
    int len = snprintf(settings_data, sizeof(settings_data),
        "U&%s&%s&!"//user_name=%s&device_name=%s //USER SETTINGS

        "S&%u&%u&%u&%u&"//first_boot=%u&device_lang=%u&units=%u&theme=%u& //SYSTEM PREFS
        "%u&"//lower_bt_volume_on_walkie
        "%u&"//auto_bt_on_powerup
        "%u&"//device_auto_record
        "%u&!"//device_mute_record_audio

        "M&%u&%u&"//photos_count=%u&videos_count=%u //MEDIA
        "%u&%u&%s&!"//bt_connected=%u&bt_audio_active=%u&bt_device_name=%s&

        "W&%u&%u&%u&!"//walkie_peers_count=%u&walkie_active=%u&walkie_paired=%u& //WALKIE
        
        "G&%.1f&%u&%.1f&!"//top_speed=%.1f&top_height=%u&top_accel=%.1f& //GPS

        "F&%s&%s&%s&"//manufacturer=%s&model_name=%s&serial_number=%s& //MANUFACTURE 
        "%s&%s&%u&!"//hardware_version=%s&led_hw_version=%s&has_leds=%u&

        "R&%u&%s&%s&"//has_remote=%u&remote_hw_version=%s&remote_chip=%s& //REMOTE
        "%s&%s&!"//remote_firmware=%s&remote_serial=%s&
        
        "B&%s&%s&"//firmware_ver_mcu=%s&firmware_ver_mpu=%s& //FIRMWARE
        "%s&%s&%s&!",//build_date=%s&build_time=%s&build_type=%s&,
        
        //USER
        "Yann",g_user_prefs.device_name,

        //SYSTEM
        g_system_prefs.first_boot, g_system_prefs.device_lang, g_system_prefs.units, g_system_prefs.theme,
        g_system_prefs.lower_bt_volume_on_walkie, g_system_prefs.auto_bt_on_powerup,
        g_system_prefs.device_auto_record, g_system_prefs.device_mute_record_audio,

        //MEDIA
        g_device_state.photos_count, g_device_state.videos_count,
        g_device_state.bt_connected, g_device_state.bt_audio_active, g_device_state.bt_device_name,

        //WALKIE
        g_device_state.walkie_peers_count, g_device_state.walkie_active, g_device_state.walkie_paired,

        //GPS
        g_device_state.top_speed, g_device_state.top_height, g_device_state.top_accel,

        //MANUFACTURE
        g_device_info.manufacturer, g_device_info.model_name, g_device_info.serial_number,
        g_device_info.hardware_version, g_device_info.led_hw_version, g_device_info.has_leds,

        //REMOTE
        g_device_info.has_remote, g_device_info.remote_hw_version, g_device_info.remote_chip,
        g_device_info.remote_firmware, g_device_info.remote_serial,

        //FIRMWARE
        g_device_info.firmware_ver_mcu, g_device_info.firmware_ver_mpu,
        g_device_info.build_date, g_device_info.build_time, g_device_info.build_type
