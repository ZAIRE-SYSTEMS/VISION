...
async function ExtractSettingsData(data) {

  let scope_1 = data.split('!');
  
  scope_1.forEach(scope=>{
    let scope_2 = scope.split('&');
    
    switch (scope_2[0]) {
      case 'U':
        userPrefs.user_name = scope_2[1];
        userPrefs.device_name = scope_2[2];
        break;
      
      case 'S':
        systemSettings.first_boot = scope_2[1];
        systemSettings.device_lang = scope_2[2];
        systemSettings.units = scope_2[3];
        systemSettings.theme = scope_2[4];
        systemSettings.lower_bt_volume_on_walkie = scope_2[5];
        systemSettings.device_auto_record = scope_2[6];
        systemSettings.device_mute_record_audio = scope_2[7];
        break;
      
      case 'M':
        mediaSettings.photos_count = scope_2[1];
        mediaSettings.videos_count = scope_2[2];
        mediaSettings.bt_connected = scope_2[3];
        mediaSettings.bt_audio_active = scope_2[4];
        mediaSettings.bt_device_name = scope_2[5];
        break;

      case 'W':
        walkieSettings.walkie_peers_count = scope_2[1];
        walkieSettings.walkie_active = scope_2[2];
        walkieSettings.walkie_paired = scope_2[3];
        break;

      case 'G':
        gpsSettings.top_speed = scope_2[1];
        gpsSettings.top_height = scope_2[2];
        gpsSettings.top_accel = scope_2[3];
        break;

      case 'F':
        manufactureInfo.manufacturer = scope_2[1];
        manufactureInfo.model_name = scope_2[2];
        manufactureInfo.serial_number = scope_2[3];
        manufactureInfo.hardware_version = scope_2[4];
        manufactureInfo.led_hw_version = scope_2[5];
        manufactureInfo.has_leds = scope_2[6];
        break;

      case 'R':
        remoteInfo.has_remote = scope_2[1];
        remoteInfo.remote_hw_version = scope_2[2];
        remoteInfo.remote_chip = scope_2[3];
        remoteInfo.remote_firmware = scope_2[4];
        remoteInfo.remote_serial = scope_2[5]
        break;

      case 'B':
        firmwareInfo.firmware_ver_mcu = scope_2[1];
        firmwareInfo.firmware_ver_mpu = scope_2[2];
        firmwareInfo.build_date = scope_2[3];
        firmwareInfo.build_time = scope_2[4];
        firmwareInfo.build_type = scope_2[5];
        break;
...
var systemSettings = {//S
  ...
  units: null,
  theme: null,
  lower_bt_volume_on_walkie: null,
  device_auto_record: null,
  device_mute_record_audio: null,
};
...
var mediaSettings = {//M
  ...
  videos_count: 0,
  bt_connected: null,
  bt_audio_active: null,
  bt_device_name: null,
};

var walkieSettings = {//W
  walkie_peers_count: null,
  walkie_active: null,
  walkie_paired: null,
};
...
var firmwareInfo = {//B
  firmware_ver_mcu: null,
  firmware_ver_mpu: null,
  build_date: null,
  build_time: null,
  build_type: null,
};
